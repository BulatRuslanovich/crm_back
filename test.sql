...
-- Подтверждаем завершенные визиты (прошедшие более 3 дней назад)
UPDATE activ 
SET status_id = 4, updated_by = 'system', updated_at = CURRENT_TIMESTAMP
WHERE visit_date < CURRENT_DATE - INTERVAL '3 days';

-- Выводим статистику для проверки
SELECT '=== СТАТИСТИКА ТАБЛИЦ ===' as info;
SELECT 'Пользователи' as table_name, COUNT(*) as count FROM usr
UNION ALL
SELECT 'Политики', COUNT(*) FROM policy
UNION ALL
SELECT 'Назначения политик', COUNT(*) FROM usr_policy
UNION ALL
SELECT 'Организации', COUNT(*) FROM org
UNION ALL
SELECT 'Статусы', COUNT(*) FROM status
UNION ALL
SELECT 'Активности (только для представителей)', COUNT(*) FROM activ
UNION ALL
SELECT 'Планы (только для менеджеров)', COUNT(*) FROM plan;

-- Показываем распределение активностей по ролям
SELECT 
    CASE 
        WHEN up.policy_id = 1 THEN 'Админ'
        WHEN up.policy_id = 2 THEN 'Руководитель'
        WHEN up.policy_id = 3 THEN 'Менеджер'
        WHEN up.policy_id = 4 THEN 'Представитель'
    END as role,
    COUNT(*) as users_count
FROM usr_policy up
GROUP BY up.policy_id
ORDER BY up.policy_id;

-- Показываем примеры данных
SELECT '=== ПРИМЕРЫ ПОЛЬЗОВАТЕЛЕЙ ПО РОЛЯМ ===';
SELECT 
    u.usr_id, 
    ...
    END as role
FROM usr u
JOIN usr_policy up ON u.usr_id = up.usr_id
ORDER BY up.policy_id, u.last_name
LIMIT 10;

SELECT '=== ПРИМЕРЫ ОРГАНИЗАЦИЙ ===';
SELECT org_id, name, inn FROM org ORDER BY name LIMIT 5;

SELECT '=== ПРИМЕРЫ АКТИВНОСТЕЙ (ТОЛЬКО ПРЕДСТАВИТЕЛИ) ===';
SELECT 
    u.first_name || ' ' || u.last_name as representative,
...
FROM activ a
JOIN usr u ON a.usr_id = u.usr_id
JOIN org o ON a.org_id = o.org_id
JOIN status s ON a.status_id = s.status_id
WHERE u.usr_id BETWEEN 11 AND 20
ORDER BY a.visit_date DESC, a.start_time
LIMIT 15;

SELECT '=== ПРИМЕРЫ ПЛАНОВ (ТОЛЬКО МЕНЕДЖЕРЫ) ===';
SELECT 
    u.first_name || ' ' || u.last_name as manager,
    o.name as organization,
    p.start_date,
    p.end_date,
    (p.end_date - p.start_date) as duration_days
FROM plan p
JOIN usr u ON p.usr_id = u.usr_id
JOIN org o ON p.org_id = o.org_id
WHERE u.usr_id BETWEEN 4 AND 10
ORDER BY p.start_date
LIMIT 10;

SELECT '=== ДАННЫЕ УСПЕШНО ЗАГРУЖЕНЫ ===';